(function(){
  ...
  async function showAuthGate(){
    ...
    gate.innerHTML='<div class="boostAuthCard"><div class="boostAuthHead"><h2>Welcome to Northern BOOST</h2><p>Sign in once so your career journey can follow you across devices.</p></div><div class="boostAuthBody"><div class="boostAuthRow"><div><label for="boostFirst">First name</label><input id="boostFirst" autocomplete="given-name"></div><div><label for="boostLast">Last name</label><input id="boostLast" autocomplete="family-name"></div></div><label for="boostEmail">Email</label><input id="boostEmail" type="email" autocomplete="email" placeholder="you@example.com"><button class="boostAuthBtn" id="boostSend">Send My Sign-In Email</button><div class="boostAuthCode" id="boostCodeWrap"><label for="boostCode">Sign-in code</label><input id="boostCode" inputmode="numeric" autocomplete="one-time-code" maxlength="8" placeholder="Enter code"><button class="boostAuthBtn" id="boostVerify">Verify Code & Start</button></div><div class="boostAuthStatus" id="boostAuthStatus"></div><div class="boostAuthNote">Open the sign-in email we send you. Use the button or link to return here. If the button does not open, come back to this screen and enter the full code shown in the email.</div><button class="boostAuthBtn secondary" id="boostDevice">Continue on this device for now</button></div></div>';
