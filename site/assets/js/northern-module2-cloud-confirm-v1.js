...
function show(msg,ok){
 const el=$('saveMessage');if(!el)return;
 el.textContent=msg;
 ...
 el.style.background=ok===false?'#fff3f3':ok===true?'#eefaf3':'#ffffff';
 el.style.color=ok===false?'#8a1f1f':ok===true?'#165c36':'#17324d';
 el.style.border=ok===false?'1px solid #e6b8b8':ok===true?'1px solid #b9dfc8':'1px solid #c9d6df';
}
async function saveDirect(){
 ...
 if(sessionError)return{ok:false,reason:'session-error',error:sessionError};
 if(session?.user){
   ...
   if(error)return{ok:false,reason:'authenticated-rpc-failed',error};
   if(data)localStorage.setItem(CLOUD_ID_KEY,String(data));
   return{ok:true,data,mode:'authenticated',email:session.user.email||''};
 }
 ...
 if(error)return{ok:false,reason:'resume-rpc-failed',error};
...
async function confirmCloudSave(){
 ...
 let result;
 try{result=await saveDirect()}catch(error){result={ok:false,reason:'unexpected-error',error}}
 if(result?.ok){show(`Module 2 complete — saved to BOOST cloud ✓${result.mode==='authenticated'?' Signed-in participant confirmed.':''}`,true);document.dispatchEvent(new CustomEvent('boostprogress',{detail:{moduleId:'module2',status:'complete'}}));return}
 const reason=result?.reason,detail=String(result?.error?.message||'').trim();
 if(reason==='identity-missing'){
   show('Module 2 is saved on this device, but this browser does not have an active participant identity. The staff dashboard cannot attach this save to a participant yet.',false);
 }else if(reason==='resume-credentials-missing'){
   show('Module 2 is saved on this device, but this browser does not have the BOOST resume credentials needed to update its cloud record.',false);
 }else if(reason==='resume-rpc-failed'){
   show(`Module 2 is saved locally, but the anonymous/resume cloud record could not be updated${detail?`: ${detail}`:'.'}`,false);
 }else if(reason==='authenticated-rpc-failed'||reason==='session-error'){
   show(`Your participant sign-in was detected, but the cloud save failed${detail?`: ${detail}`:'.'}`,false);
 }else if(reason==='cloud-not-configured'){
   show('Module 2 is saved locally, but BOOST cloud is not configured on this page.',false);
 }else{
   show(`Module 2 saved locally, but BOOST cloud did not confirm the save${detail?`: ${detail}`:'.'}`,false);
 }
